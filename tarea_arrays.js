...
function procesarPedido(pedido) {
  ...
  pedido.push(nombreCliente); // Añade el nombre del cliente al final del array
  return pedido; 
}
...
function sumarPares(numeros) {
    ...
    return suma;// aca retorna la suma 
...
for (let i = 0; i < palabras.length; i++) 
        {
        if (!palabras[i].endsWith('a')) //retornan las palabras que terminan con a
             {
            return false;
            }
    }
...
function buscaPalabras(words, word) {
    ...
    return words.slice(palabra + 1);  // Devolver palabras después del índice
}
